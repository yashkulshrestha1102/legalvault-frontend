...
export default function RenameModal({ open, onClose, onRename, currentName, type = 'item' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err.message || 'Failed to rename');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      <div className="glass w-full max-w-md p-6 rounded-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="glass-icon text-yellow-400">
            <FaEdit />
          </div>
          <h2 className="text-xl font-bold">Rename {type === 'folder' ? 'Folder' : 'File'}</h2>
        </div>
