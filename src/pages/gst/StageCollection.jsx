...
import { FaUpload, FaSpinner, FaFileInvoice, FaTrash, FaArrowRight } from "react-icons/fa";
  ...
  return (
    <MainLayout>
      <div className="glass-card p-6 mb-8">
        ...
        <p className="text-white/60 mt-1">Client: {automation?.clientId?.name || 'N/A'} | GSTIN: {automation?.gstin || 'N/A'}</p>
        <p className="text-white/40 text-sm">Status: <span className="capitalize">{automation?.status}</span></p>
        <p className="text-white/40 text-sm">Documents: <span className="text-white font-bold">{documents.length}</span></p>
      </div>
      ...
      <div className="glass-card p-6 mb-8">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <input id="fileInput" type="file" onChange={handleFileChange} className="flex-1 bg-slate-800/50 border border-white/10 rounded-xl px-4 py-3 text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-cyan-500/20 file:text-cyan-400" />
          <button onClick={handleUpload} disabled={!selectedFile || uploading} className="glass-card px-6 py-3 flex items-center justify-center gap-2 text-white disabled:opacity-50">
            {uploading ? <FaSpinner className="animate-spin" /> : <FaUpload />}
            {uploading ? 'Uploading...' : 'Upload'}
          </button>
        </div>
        {selectedFile && <p className="mt-2 text-sm text-white/40">Selected: {selectedFile.name}</p>}
      </div>
        ...
        {documents.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {documents.map((doc, index) => (
              <div key={index} className="bg-slate-800/30 border border-white/10 rounded-xl p-4 flex items-center gap-3">
      ...
      <div className="flex justify-end">
        <button onClick={handleNext} disabled={documents.length === 0} className="glass-card px-8 py-3 flex items-center gap-2 text-white disabled:opacity-50">
