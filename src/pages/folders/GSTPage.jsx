...
const GSTPage = ({ clientId }) => {
  ...
  const fetchGST = async () => {
    try {
      ...
      const response = await api.get(`/api/gst/client/${clientId}`);
  ...
  const saveGST = async (data) => {
    try {
      ...
      if (editData) {
        await api.put(`/api/gst/${editData._id}`, payload);
      } else {
        await api.post('/api/gst', payload);
      }
