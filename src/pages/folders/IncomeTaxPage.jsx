...
import AddIncomeTaxModal from '../../components/modals/AddIncomeTaxModal';

const API_URL = 'https://legalvault-jm2n.onrender.com';

const IncomeTaxPage = ({ clientId }) => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/income-tax/client/${clientId}`, {
  ...
  const saveRecord = async (data) => {
    try {
      ...
      if (editData) {
        await axios.put(`${API_URL}/api/income-tax/${editData._id}`, payload, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      } else {
        await axios.post(`${API_URL}/api/income-tax`, payload, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      }
      ...
      setOpenModal(false);
      setEditData(null);
      alert('✅ Income Tax record saved successfully!');
    } catch (error) {
  ...
  const deleteRecord = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`${API_URL}/api/income-tax/${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchRecords();
  ...
  const filteredRecords = records.filter(r =>
    ...
    r.taxType?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="glass p-6">
        <div className="flex justify-center items-center h-32">
          <div className="text-center">
            ...
            <p className="text-gray-400">Loading Income Tax records...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="glass p-6">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <h2 className="text-2xl font-bold">💰 Income Tax</h2>
        ...
        <table className="w-full min-w-[900px]">
          <thead className="bg-white/5">
            <tr className="border-b border-white/10">
              <th className="p-3 text-left text-sm">Tax Name</th>
              <th className="p-3 text-left text-sm">Type</th>
              <th className="p-3 text-left text-sm">PAN</th>
              <th className="p-3 text-left text-sm">Assessment Year</th>
            ...
            {filteredRecords.length === 0 ? (
              ...
              filteredRecords.map((record) => (
                <tr key={record._id} className="border-b border-white/5 hover:bg-white/5 transition">
                  <td className="p-3 font-medium">{record.taxName}</td>
                  <td className="p-3">
                    <span className="glass-card px-2 py-1 text-xs">{record.taxType}</span>
                  </td>
                  <td className="p-3 text-sm font-mono">{record.panNumber || '-'}</td>
                  <td className="p-3 text-sm">{record.assessmentYear || '-'}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <div className="flex gap-3">
                      <button
                        onClick={() => navigate(`/client/${clientId}/income-tax/${record._id}`)}
