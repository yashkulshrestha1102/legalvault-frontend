...
const IncomeTaxDetails = () => {
  ...
  useEffect(() => {
    const fetchRecord = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/api/income-tax/${id}`, {
  ...
  return (
    <div className="glass p-6">
      ...
      <h1 className="text-3xl font-bold mb-6">{record.taxName}</h1>

      <div className="grid md:grid-cols-2 gap-4 mb-6">
        <div className="glass-card p-4">
          <p className="text-gray-400 text-sm">Tax Type</p>
          <h3 className="font-semibold mt-1">{record.taxType || '-'}</h3>
        </div>
        <div className="glass-card p-4">
          <p className="text-gray-400 text-sm">PAN Number</p>
          <h3 className="font-mono font-semibold mt-1">{record.panNumber || '-'}</h3>
        </div>
        <div className="glass-card p-4">
          <p className="text-gray-400 text-sm">Assessment Year</p>
          <h3 className="font-semibold mt-1">{record.assessmentYear || '-'}</h3>
        </div>
        <div className="glass-card p-4">
            ...
            {record.pdfs.map((pdfUrl, index) => (
              <div key={index} className="glass-card p-4 hover:scale-105 transition">
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-sm font-medium text-center truncate w-full">Income Tax {index + 1}</p>
