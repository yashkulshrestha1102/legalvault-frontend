...
const HRPage = ({ clientId }) => {
  ...
  const fetchRecords = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/api/hr/client/${clientId}`, {
  ...
  const saveRecord = async (data) => {
    try {
      ...
      if (editData) {
        await axios.put(`${API_URL}/api/hr/${editData._id}`, payload, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      } else {
        await axios.post(`${API_URL}/api/hr`, payload, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      }
      ...
      setOpenModal(false);
      setEditData(null);
      alert('✅ HR record saved successfully!');
    } catch (error) {
  ...
  const filteredRecords = records.filter(r =>
    ...
    r.hrType?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="glass p-6">
        <div className="flex justify-center items-center h-32">
          <div className="text-center">
            ...
            <p className="text-gray-400">Loading HR records...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="glass p-6">
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <h2 className="text-2xl font-bold">👥 HR Records</h2>
        ...
        <table className="w-full min-w-[900px]">
          <thead className="bg-white/5">
            <tr className="border-b border-white/10">
              <th className="p-3 text-left text-sm">HR Name</th>
              <th className="p-3 text-left text-sm">Type</th>
              <th className="p-3 text-left text-sm">Employee ID</th>
              <th className="p-3 text-left text-sm">Department</th>
            ...
            {filteredRecords.length === 0 ? (
              ...
              filteredRecords.map((record) => (
                <tr key={record._id} className="border-b border-white/5 hover:bg-white/5 transition">
                  <td className="p-3 font-medium">{record.hrName}</td>
                  <td className="p-3">
                    <span className="glass-card px-2 py-1 text-xs">{record.hrType}</span>
                  </td>
                  <td className="p-3 text-sm font-mono">{record.employeeId || '-'}</td>
                  ...
                  <td className="p-3">
                    <div className="flex gap-3">
                      <button
                        onClick={() => navigate(`/client/${clientId}/hr/${record._id}`)}
