...
const deleteClient = (indexToDelete) => {
  ...
  addNotification(
  `Client Deleted: ${clients[indexToDelete].name}`
);
...
<div className="overflow-x-auto">
        <table className="min-w-[900px] w-full">
          ...
          <tbody>
            {clients.filter((client) =>client.name.toLowerCase().includes(searchTerm.toLowerCase())).map((client, index) => (
              <tr
                key={index}
                className="
border-b
...
/>
                    ...
                    <FaEdit className=" cursor-pointer text-yellow-400 hover:scale-125 transition-all" onClick={() => {
                    setEditIndex(index);
                    setEditData(client);
                    setOpenModal(true); }}/>

                    <FaTrash className=" cursor-pointer text-red-500 hover:scale-125 transition-all" onClick={() => deleteClient(index)}/>
                  </div>
                </td>
              </tr>
            ))}
