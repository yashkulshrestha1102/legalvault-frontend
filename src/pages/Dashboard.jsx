...
function Dashboard() {
  ...
  const [stats, setStats] = useState({  // ✅ setStats define karo
    totalClients: 0,
    activeCases: 0,
    documents: 0,
    consultants: 0
  ...
  // Local storage se clients count (fallback)
  useEffect(() => {
    const clients = JSON.parse(localStorage.getItem("clients")) || [];
    if (clients.length > 0 && clientCount === 0) {
  ...
  return (
    <MainLayout>
      ...
      <div className="relative overflow-hidden rounded-3xl p-6 md:p-8 mb-8 glass-card backdrop-blur-3xl border border-white/10 bg-white/5">
        <div className="relative z-10">
          ...
          <p className="text-blue-100 text-sm md:text-lg leading-7">
            Manage Legal Cases, Clients, Policies and Documents.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <StatsCard
          title="Total Clients"
          value={stats.totalClients || clientCount}
          growth="12"
          icon={<FaUsers />}
        />
      ...
      <div className="glass-card p-6 rounded-2xl">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <button
            ...
            className="glass-card p-5 hover:scale-105 hover:border-cyan-400/40 transition-all duration-300"
          >
            + Add Client
          </button>
